...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    if path.endswith('.npz'):
        ...
        files = []
        for i in range(len(hp.category)):
            if not os.path.exists(path + hp.category[i]):
                cat = hp.category[i].split(".")[0]
            else:
                cat = hp.category[i]
            path_cat = pathlib.Path(path+cat)
            files = files + list(path_cat.glob('*.jpg')) + list(path_cat.glob('*.png'))
