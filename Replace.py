...
class tester:
    ...
    def batch_test(self, batch_data, j):
        ...
        for idx in range(len(sketch_stroke_idx)):
            tensor = np.load(f'./stroke/' + str(template_idx) + '/' + str(template_stroke_idx[idx]) + '.npy')
            tensor = torch.from_numpy(tensor).to(torch.float32)
            tensor = tensor.cuda(z.get_device())
            z[0][sketch_stroke_idx[idx]] = (tensor - z[0][sketch_stroke_idx[idx]]) / 10 * j + z[0][sketch_stroke_idx[idx]]
